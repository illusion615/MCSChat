 ...
 * 
 * ⚠️  DEPRECATED & MOVED TO LEGACY: This file has been moved to the legacy folder.
 * This implementation is no longer maintained and should NOT be used in new projects.
 * 
 * 🚫 DO NOT USE - Use one of these instead:
 * 
 * Current (Simple): import { DirectLineManager } from '../src/components/directline/DirectLineManagerSimple.js';
 * Enhanced (Future): import { EnhancedDirectLineManager } from '../src/components/directline/DirectLineManagerEnhanced.js';
 * 
 * This legacy version has been preserved for historical reference only.
 * 
 * Moved on: August 24, 2025
 * Original location: src/services/directLineManager.js
 * New location: legacy/directLineManager-legacy.js
 */
...
export class DirectLineManager {
    ...
    handleStreamingChunk(activity) {
        // Check if this is the first bot message (greeting) starting to stream
        if (!this.greetingReceived && activity.text && activity.text.trim().length > 0) {
            console.log('First bot message streaming - greeting detected');
    ...
    sendGreeting() {
        ...
        setTimeout(() => {
            try {
                ...
                if (chatHistoryData && typeof chatHistoryData === 'string') {
                    chatHistory = JSON.parse(chatHistoryData);
                }

                const currentSession = sessionStorage.getItem('currentSession') || 'default';
                const currentSessionMessages = chatHistory.filter(entry => entry.session === currentSession);
                const botMessages = currentSessionMessages.filter(msg => msg.sender === 'bot');
                ...
                console.error('Error checking greeting timeout:', error);
                document.dispatchEvent(new CustomEvent('agent:greeting:timeout'));
            }
        }, 5000);
    }
